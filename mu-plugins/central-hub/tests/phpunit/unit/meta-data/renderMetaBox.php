...
class Tests_RenderMetaBox extends Test_Case {
	...
	public function test_should_assign_meta_box_id_to_html_field_names() {
		// Set up the mocks.
		Monkey\Functions\when( 'KnowTheCode\ConfigStore\getConfig' )->justReturn( $this->meta_config );
		Monkey\Functions\when( 'spiralWebDB\Metadata\get_custom_fields_values' )->justReturn( $this->meta_config['custom_fields'] );
...
NONCE;
	...
	public function test_should_render_the_custom_field_values() {
		...
		Monkey\Functions\expect( 'spiralWebDB\Metadata\get_custom_fields_values' )
			->once()
			->with( $this->post->ID, 'events', $this->meta_config )
			->andReturn( [
